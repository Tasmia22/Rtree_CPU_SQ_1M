...
#define BUNDLEFACTOR 4 // Number of points to form a leaf node
#define FANOUT 5       // Number of children per non-leaf node
...
bool searchSerializedTree(SerializedNode *serialized_tree, int current_index, Point queryPoint)
{
    ...
    else
    {
        ...
        for (int i = 0; i < current_node->count; i++)
        {
            printf("\nCurrent node=%d and searching its child=%d\n\n",current_index,current_node->children[i]);
